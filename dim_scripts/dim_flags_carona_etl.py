...
# Isso deve ser consistente com o que foi usado na pré-população da dim_flags_carona
DAY_NUM_TO_FLAG_COL = {
    1: 'is_routine_monday',
...
# Ordem das flags para lookup na dim_flags_carona (DEVE SER A MESMA DA CRIAÇÃO DA DIMENSÃO SUCATA)
...
# Variável global para armazenar o lookup da dim_flags_carona
# Será populada uma vez por execução do ETL
_FLAGS_CARONA_LOOKUP_DICT = {}

def load_flags_carona_lookup(conn_dw):
    """Carrega a dim_flags_carona para um dicionário de lookup em memória."""
    global _FLAGS_CARONA_LOOKUP_DICT

    if _FLAGS_CARONA_LOOKUP_DICT: # Já carregado
        return

    print("Carregando dim_flags_carona para lookup em memória...")
    try:
        query = f"SELECT {', '.join(FLAG_NAMES_ORDER)}, flags_carona_sk FROM dim_flags_carona;"
        flags_df = pd.read_sql(query, conn_dw)

        # Cria o dicionário de lookup: (True, False, ..., True) -> flags_carona_sk
        _FLAGS_CARONA_LOOKUP_DICT = {
            tuple(row[col] for col in FLAG_NAMES_ORDER): row['flags_carona_sk']
            for index, row in flags_df.iterrows()
        }
        print(f"dim_flags_carona carregada: {len(_FLAGS_CARONA_LOOKUP_DICT)} combinações.")
    except Exception as e:
        print(f"Erro ao carregar dim_flags_carona para lookup: {e}")
        _FLAGS_CARONA_LOOKUP_DICT = {} # Limpa em caso de erro para tentar novamente se necessário

def derive_and_lookup_flags(row_oltp):
    """
    Deriva as flags booleanas para uma carona do OLTP e busca o flags_carona_sk correspondente.
    """
    ...
    flags_carona_sk = _FLAGS_CARONA_LOOKUP_DICT.get(flags_tuple)
    
    if flags_carona_sk is None:
...
def etl_dim_flags_carona():
    ...
    try:
        print("Truncando dim_flags_carona antes da pré-população...")
        with conn_dw.cursor() as cur:
            cur.execute("TRUNCATE TABLE dim_flags_carona RESTART IDENTITY CASCADE;")
        conn_dw.commit()
        print("dim_flags_carona truncada com sucesso.")
        ...
        print(f"Gerados {len(data_to_load)} registros logicamente válidos para dim_flags_carona.")
